...
export function categoryPresentation(input: {
  ...
  if (currencies.length === 0) {
    return { currencies: [], rows: [], emptyMessage: NO_CATEGORIES_MESSAGE };
  }
  ...
  const rows: CategoryRow[] = ranked
    .slice(0, 5)
    .map((e) => ({ categoryId: e.categoryId, name: e.name, amount: money(e.amount, selectedCurrency) }));
  const rest = ranked.slice(5);
  const remainder: CategoryRemainder | undefined =
    rest.length > 0
      ? {
          count: rest.length,
          label: `Ще ${rest.length}`,
          amount: money(rest.reduce((sum, e) => sum + e.amount, 0), selectedCurrency),
        }
      : undefined;
