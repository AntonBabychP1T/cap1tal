...
import { CONTEXT, INSTRUCTIONS, type PromptProfile } from './prompt';
 ...
 *
 * Four sections, in the order they have to be read in. **Інструкції** — what to do with the data
 ...
 * anywhere — and by a model second, and headings are what carry the four sections to both.
 *
...
export interface AnalysisDocument {
  ...
  /** Instructions + context + summary + data, in that order. */
...
export function renderDocument(
  ...
  if (packaged.goals.length > 0) {
    ...
    for (const goal of packaged.goals) {
      const state = goal.reached ? 'досягнута' : goal.overdue ? 'прострочена' : `до ${goal.deadline}`;
