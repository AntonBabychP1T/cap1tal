...
app may use (vision §14.7) — those transactions are still typed by hand or lost. Step 8 of
...
- No per-bank parsers yet: real notification texts are the owner's personal data on the
  ...
  as samples do.
- No SMS parsing and no other banks' APIs (vision §14.7); no remote push delivery, reminders or
  operational alerts in this engine (vision §§13–14); this change only reads notifications from
  other apps.
