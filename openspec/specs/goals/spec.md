...
A ціль is something the owner is aiming at, of exactly one of two kinds, chosen when it is
created and never changed afterwards. A ціль-накопичення — «накопичити N» — holds a назва, a
target сума in its own currency, an optional дата and a склад of one or more рахунки; its
прогрес is the sum of those рахунки's внески, read live at the moment it is shown and never a
second, hand-maintained number that could drift from the stored truth. A ціль витрат —
«витратити не більше N цього місяця» — is one категорія's ліміт read as a ціль, holding no сума,
currency or назва of its own. The two kinds mean opposite things and share no progress
vocabulary: nothing calls a ціль витрат досягнута, and nothing calls a ціль-накопичення
перевищена.
